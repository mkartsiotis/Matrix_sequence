                                    ...
                                    for (int i = h; i <= 9; i++)
                                        for (int j = i; j <= 9; j++)
                                        {
                                            ...
                                            for (int k = 0; k < Num_of_repetitions; k++)
                                            {
                                                ...
                                                if (is_circular(k) == 0)
                                                    ; // counting non_convergent sequences
                                                else
                                                {
...
void check_and_save(unsigned long long int n)
{
    ...
    // We now need to perform some checks....
    for (size_t i = 0; i <= count; i++)
    {
        if (results[i] == n) // checking if we have stored the number previously to avoid extra saves
            return;          // skip if found already
    }
